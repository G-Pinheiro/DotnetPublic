...
namespace CargoShipLoader
{
    public class Ship
    {
        ...
        public Ship()
        {
            ...
            Capacity = (random.Next(MAX_WEIGHT) * CYCLE_WEIGHT) + (random.Next(MAX_WEIGHT) * CAR_WEIGHT) + (random.Next(MAX_WEIGHT) * TRUCK_WEIGHT)
                + (random.Next(MAX_WEIGHT) * TRAIN_WEIGHT);
        }

        // Calculate the capacity after user inputs
        public int overUnder()
        {
            return Capacity - getShipLoad();
        }

        // Override ToString standard method
